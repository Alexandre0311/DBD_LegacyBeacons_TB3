...
--Queries
-----------------------------Alejandro -----------------------------
-- Seleccionar los proyectos con nombres
select nombre, nombre_comun
from especies_proyectos
inner join proyectos as P on especies_proyectos.id_proyecto = P.id_proyecto
inner join especies as E on especies_proyectos.id_especie = E.id_especie

go

-- Funcion que devuelve los habitats de un ecosistema
create function FTHabitats_De_Un_Ecosistema(@ecosistema nvarchar(40)) returns table
return
select h.descripcion from habitats_ecosistemas as he
inner join habitats as h on he.id_habitat = h.id_habitat
inner join ecosistemas e on he.id_ecosistema = e.id_ecosistema
where tipo_ecosistema=@ecosistema
go
select *
from FTHabitats_De_Un_Ecosistema ('Bosques de Niebla' )
go


-- Proceso almacenado que permita actualizar la tabla especies , los valores de cantidad de ejemplares y estaod de conservacion
create procedure PRCActualizar_especie
@id_especie int,
@cantidad int,
@estado nvarchar(30)
as
begin
   begin try
       begin transaction
       update especies
       set cantidad_ejemplares=@cantidad, estado_conservacion=@estado
...
go
PRCActualizar_especie @id_especie = 01, @cantidad = 120, @estado = N'Peligro de extinción'
go
--Proceso almacenado que devuelva la cantidad de graficos de una especie por su nombre comun
create view VNombreEspecieporGrafico as
select nombre_comun,count(*) as Cantidad_Graficos
   from graficos
   inner join especies e on graficos.id_especie = e.id_especie
   group by nombre_comun
go


create procedure PRCCantidadGraficoPorEspecie
@Cantidad int OUTPUT,
@Especie nvarchar(30)
as
begin
   set @Cantidad=(select Cantidad_Graficos
   from VNombreEspecieporGrafico where nombre_comun=@Especie)
end
go
declare @Can int
exec PRCCantidadGraficoPorEspecie   @Cantidad = @Can output , @Especie = 'Oso Andino'
select @Can
go
-------------------Mathias---------------------------------------------
--muestra las especies en peligro de extincion
select * from especies
where estado_conservacion = N'Peligro de extinción'
go

--muestra los depredadores para una determinada especie
create function f_depredador_por_especie(@nombre_especie varchar(100)) returns table
return
select id_depredador from especies_depredadores as e_d
inner join especies as e on e_d.id_especie = e.id_especie
where e.nombre_comun = @nombre_especie
or e.nombre_cientifico = @nombre_especie
go
create function f_depredadores_por_especie(@nombre_especie varchar(100)) returns table
return
select * from especies
where id_especie  in (select id_depredador from f_depredador_por_especie(@nombre_especie))
go
select * from f_depredadores_por_especie('Gato de los Andes')
go
--buscar especie
create procedure ups_especie_por_texto
@nombre_especie varchar(100)
as begin
select * from especies
where nombre_comun like concat('%',@nombre_especie,'%')
or nombre_cientifico like concat('%',@nombre_especie,'%')
end
go

--tipo de especie con mayor avistamiento para un determinado anio
create view V_avistamientos_por_especie_por_anio
as
select es.nombre_comun especie,year(fecha_avistamiento) anio, count(id_avistamiento) total_avistamientos from avistamientos as a
inner join especies es on a.id_especie = es.id_especie
group by es.nombre_comun, year(fecha_avistamiento)
go

create procedure UPS_especies_con_maximos_avistamientos_por_anio
@anio int
as begin
select especie,total_avistamientos  from V_avistamientos_por_especie_por_anio
where total_avistamientos = (select max(total_avistamientos) from V_avistamientos_por_especie_por_anio
                           where anio =  @anio)
and anio =  @anio
end
go
---total especies avistadas en un determinado anio
create function f_especies_avistdas_por_anio(@anio int) returns int
as begin
return (select sum(cantidad_afectada)  from avistamientos
       where year(fecha_avistamiento) = @anio)
end
go


--Mostrar la especie con la menor cantidad de ejemplares
select nombre_cientifico, nombre_comun,cantidad_ejemplares
from especies
where cantidad_ejemplares=(select min(cantidad_ejemplares)from especies)

go

----------------------------Jimena--------------------------------
-- Mostar a las especies y sus respectivas amenazas
select e.nombre_comun, a.nombre as amenaza
from especies e
		inner join amenazas a on e.id_especie = a.id_amenaza;
go

-- Mostrar la especie con la menor cantidad de ejemplares
select nombre_cientifico, nombre_comun, cantidad_ejemplares
from especies
where cantidad_ejemplares = (
    select min(cantidad_ejemplares)
    from especies
);
go

-- Mostrar el habitat de cada especie
create function FHabitatxEspecie(@nombre_especie varchar (50)) returns table
return
select h.descripcion as 'habitat'
from habitats h
	inner join especies as e on h.id_especie = e.id_especie
	where e.nombre_comun= @nombre_especie
go

select * from FHabitatxEspecie(N'Delfín rosado');
go

-- Mostrar a las especies y sus respectivas amenazas
create procedure PMostrarEspeciesAmenazas
as
begin
    select e.nombre_comun as 'Especie', a.nombre as 'Amenaza'
    from especies e
    inner join especies_amenazas ea on e.id_especie = ea.id_especie
    inner join amenazas a on ea.id_amenaza = a.id_amenaza;
end
go
exec PMostrarEspeciesAmenazas
go


-- Mostar las especies y la ecorregion en la que se encuentran
create procedure PMostrarEspeciesEcorregion
as
begin
    select especies.nombre_cientifico as Especies, ecorregiones.nombre as Ecorregion
    from especies
    inner join habitats on especies.id_especie = habitats.id_especie
    inner join habitats_ecosistemas on habitats.id_habitat = habitats_ecosistemas.id_habitat
    inner join ecorregiones_ecosistemas on habitats_ecosistemas.id_ecosistema = ecorregiones_ecosistemas.id_ecosistema
    inner join ecorregiones on ecorregiones_ecosistemas.id_ecorregion = ecorregiones.id_ecorregion;
end
go
exec PMostrarEspeciesEcorregion;
go

-- Mostrar las especies y que organizaciones las apoyan
create view VEspeciesxOrganizaciones
as
	select e.nombre_cientifico as Especie, o.nombre as Organizacion
	from especies e
		inner join especies_proyectos ep on e.id_especie = ep.id_especie
		inner join proyectos p on ep.id_proyecto = p.id_proyecto
		inner join organizaciones_proyectos op on p.id_proyecto = op.id_proyecto
		inner join organizaciones o on op.id_organizacion = o.id_organizacion;

go

create function FMostrarEspeciesOrganizaciones() returns table
as
return (
    select Especie, Organizacion
    from VEspeciesxOrganizaciones
);
go

select * from FMostrarEspeciesOrganizaciones();
go

-----------------------------Nelson----------------------------
--mostrar el estado de conservacion de las especies
select nombre_cientifico, nombre_comun, estado_conservacion
    from especies
go

--muestra los proyectos junto a su organizacion y sector asociado
select p.id_proyecto, p.nombre as Proyecto, p.descripcion as Descripcion, o.nombre as Organizacion, o.sector as Sector
from proyectos as p
inner join organizaciones_proyectos as op on p.id_proyecto = op.id_proyecto
inner join organizaciones as o on op.id_organizacion = o.id_organizacion;
go

--mostrar las organizaciones que financian los proyectos seg�n la especie
create function ProyectosFinanciadosOrganizacionesPorEspecie(@nombreComunEspecie varchar(100))
returns table
as
return (
    select org.nombre as organizacion, org.sector as sector, p.nombre as proyecto
    from especies as es
    inner join especies_proyectos as ep on es.id_especie = ep.id_especie
    inner join proyectos as p on ep.id_proyecto = p.id_proyecto
    inner join organizaciones_proyectos as op on p.id_proyecto = op.id_proyecto
    inner join organizaciones as org on op.id_organizacion = org.id_organizacion
    where es.nombre_comun = @nombreComunEspecie
);
go
select * from ProyectosFinanciadosOrganizacionesPorEspecie('Oso Andino')
go

-- mostrar las amenazas y causas que lo generan seg�n la especie
create function MostrarAmenazasPorEspecie(@nombreEspecie varchar(100))
returns table
as
return
(
    select  a.nombre as nombre_amenaza, a.causa
    from amenazas as a
    inner join especies_amenazas as ea on a.id_amenaza = ea.id_amenaza
    inner join especies as e on ea.id_especie = e.id_especie
    where e.nombre_comun = @nombreEspecie
);
go
select * from MostrarAmenazasPorEspecie('Oso Andino')
go
-------------------------------------Joaquin---------------------------
/*Proyectos que sean financiados o dirigidos por organizaciones especificas*/

SELECT *
FROM proyectos
inner join organizaciones_proyectos on proyectos.id_proyecto = organizaciones_proyectos.id_proyecto
inner join organizaciones on  organizaciones_proyectos.id_organizacion = organizaciones.id_organizacion
WHERE organizaciones.nombre IN ('Reserva Natural Andina');
go

/*Organizaciones cuya relacion a proyectos sea de financiamiento*/

SELECT *
FROM organizaciones
inner join organizaciones_proyectos op on organizaciones.id_organizacion = op.id_organizacion
WHERE tipo_relacion = 'Apoyo económico';
go
/*Agrupar y contar las organizaciones por sector especifico*/

SELECT COUNT(*) as cantidad
FROM organizaciones
WHERE sector = N'Conservación de Hábitats';
go
